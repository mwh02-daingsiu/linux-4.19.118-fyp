...
#ifdef CONFIG_EXT2FYP_FS_POSIX_ACL
...
static int ext2_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	sbi->s_bmpt_addr_per_block_bits =
		ilog2 (EXT2_BMPT_ADDR_PER_BLOCK(sb));
	...
	if (!EXT2_HAS_INCOMPAT_FEATURE(sb, EXT2_FEATURE_INCOMPAT_FYP))
		...
		sbi->s_dupinode_dup_cnt = le16_to_cpu(es->s_dupinode_dup_cnt);
	...
	for (iul = 1;
	     ...
	     iul++) {
		if (ext2_bg_has_super(sb, iul))
			sbi->s_groups_has_super[jul++] = iul;
	}
	sbi->s_ngroups_has_super = jul;
	for (i = 0; i < db_count; i++) {
			...
			for (jul = 1; jul < sbi->s_ngroups_has_super; jul++) {
				unsigned long backup_sb =
					ext2_group_first_block_no(
						sb,
						sbi->s_groups_has_super[jul]);
				block = descriptor_loc(sb, backup_sb, i);
				sbi->s_group_desc[i] = sb_bread(sb, block);
				if (sbi->s_group_desc[i])
					break;
			}
...
static int __init init_ext2_fs(void)
{
	...
	if (err)
		return err;
	err = dup_buf_kmem_init();
	if (err)
		goto out;
	err = dup_mapping_kmem_init();
	if (err)
		goto out;
